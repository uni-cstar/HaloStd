...
import halo.android.startActivitySafely


/**
 * 寻求打开网页的Intent
 */
fun Context.startEasyIntentForUrl(url: String) {
    val uri = Uri.parse(url)
    startEasyIntentForUri(uri)
}

/**
 * 使用系统浏览器打开网页
 */
fun Context.startEasyIntentForUrlBySystemBrowser(url: String) {
    val uri = Uri.parse(url)
    try {
 ...
 * 寻求打开对应数据的Intent
 */
fun Context.startEasyIntentForUri(uri: Uri) {
    val it = Intent(Intent.ACTION_VIEW)
    it.data = uri
    startActivitySafely(it)
}
 ...
 */
fun Context.startIntentForSettings(action: String, data: Uri? = null) {
