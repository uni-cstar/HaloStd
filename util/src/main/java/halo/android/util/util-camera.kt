...
import halo.kotlin.util.orDefaultIfNullOrEmpty
import java.io.File
 ...
 * 照相机是否可用
 */
fun isCameraEnable(ctx: Context): Boolean {
...
fun mediaScannerScanFile(ctx: Context, path: String) {
    val f = File(path)
    mediaScannerScanFile(ctx, Uri.fromFile(f))
}
...
@BestPerformance("invoke the system's media scanner to add your photo to the Media Provider's database,\n" +
        "making it available in the Android Gallery application and to other apps.")
@Deprecated("maybe,it does not work on some devices")
fun mediaScannerScanFile(ctx: Context, uri: Uri) {
 ...
 * 扫描指定媒体文件
 */
fun mediaScannerScanFile(ctx: Context, paths: Array<String>, mimeTypes: Array<String>?, callback: MediaScannerConnection.OnScanCompletedListener?) {
    MediaScannerConnection.scanFile(ctx, paths, mimeTypes, callback)
}

/**
 * 扫描指定媒体文件
 */
fun mediaScannerScanFile(ctx: Context, file: File, callback: MediaScannerConnection.OnScanCompletedListener?) {
    val path = file.absolutePath
    val mimeType = getMimeType(file)
    mediaScannerScanFile(ctx, arrayOf(path), arrayOf(mimeType), callback)
}

/**
 * 扫描指定媒体文件
 */
fun mediaScannerScanFile(ctx: Context, path: String, callback: MediaScannerConnection.OnScanCompletedListener?) {
    ...
    fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (this.requestCode == requestCode) {
            ...
            if (resultCode == RESULT_OK) {
                mediaScannerScanFile(helperCtx.ctx.applicationContext, arrayOf(currentPhotoFile), null, MediaScannerConnection.OnScanCompletedListener {
                    path, uri ->
                    Log.d(TAG, "拍照存储文件扫描结束：path=$path uri=$uri")
//                    uri?.let {
//                        val intent = Intent(Intent.ACTION_INSERT_OR_EDIT, uri)
//                        helperCtx.ctx.startActivity(intent)
//                    }
                })
//                mediaScannerScanFile(helperCtx.ctx, currentPhotoFile)
